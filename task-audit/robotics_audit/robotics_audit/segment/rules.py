...
from robotics_audit.common.description_utils import contains_any_word, is_plausible_description
...
def load_audit_config(config_path: Optional[str] = None) -> Dict[str, Any]:
    path = Path(config_path or Path(__file__).resolve().parent.parent.parent / "audit_config.json")
...
def analyze_elements(description: str, word_lists: Dict[str, List[str]]) -> ElementAnalysis:
    ...
    return ElementAnalysis(
        subject=contains_any_word(text, word_lists.get("subject", [])),
        action=contains_any_word(text, word_lists.get("action", [])),
        object=contains_any_word(text, word_lists.get("object", [])),
        result=contains_any_word(text, word_lists.get("result", [])),
    )
...
def check_segment_rules(
    ...
    *,
    ...
    elif is_garbage_text(desc, min_len=4):
        violations.append(
            _violation(
                "garbage_description",
                grade_map,
                f"第 {idx + 1} 段描述疑似乱填: {desc[:40]}",
                idx,
            )
        )
    elif not is_plausible_description(desc, word_lists):
        violations.append(
            _violation(
                "nonsense_description",
                grade_map,
                f"第 {idx + 1} 段描述不像有效语句，无法组成合理动作描述",
                idx,
            )
        )
