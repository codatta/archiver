...
CSV_FILE = BASE_DIR / "旋钮二次标注数据.csv"
...
def main():
    parser = argparse.ArgumentParser(description='从旋钮二次标注数据下载图片到 images 目录')
    parser.add_argument(
        '--workers',
        type=int,
    ...
    print(f"预计下载 {len(tasks) * 2} 张图片（每条记录2张：原图+标注图）")
    ...
    if error_records:
        ...
        for sid, records in sorted(error_by_submission.items()):
            ...
            print(f"  - {sid}: {', '.join(types)} - {', '.join(errors[:2])}")
