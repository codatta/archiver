...
# -*- coding: utf-8 -*-
"""
从审核结果中下载未通过的original图片：
1. 读取review_results.csv，找出审核未通过的记录（review_result == 0）
2. 从旋钮_交付.csv中获取对应的图片URL
3. 只下载original_image
4. 图片命名为: submission_id + "_original"
5. 下载的图片存储到images_reject目录下
"""
...
OUTPUT_DIR = BASE_DIR / "images_reject"
LOG_DIR = BASE_DIR / "logs"
LOG_FILE = LOG_DIR / f"download_rejected_{datetime.now().strftime('%Y%m%d_%H%M%S')}.log"
...
def download_image(url, save_path, submission_id):
    """下载单张图片"""
    # 下载前检查文件是否已存在
    if save_path.exists():
        logger.debug(f"文件已存在，跳过: {save_path.name} (submission_id: {submission_id})")
    ...
    try:
        ...
        logger.debug(f"下载成功: {save_path.name} (submission_id: {submission_id})")
        return {'status': 'success', 'path': save_path}
    except Exception as e:
        error_msg = f"下载失败 - submission_id: {submission_id}, url: {url}, error: {str(e)}"
        logger.error(error_msg)
        return {'status': 'error', 'error': str(e), 'url': url}

def load_rejected_submission_ids():
    """加载审核未通过的submission_id列表"""
    rejected_ids = set()
    ...
    with open(REVIEW_RESULT_CSV, 'r', encoding='utf-8') as f:
        ...
        for row in reader:
            if row.get('review_result') == '0':
                rejected_ids.add(row['submission_id'])
    
    logger.info(f"从审核结果中读取到 {len(rejected_ids)} 个未通过的记录")
    return rejected_ids
...
def download_single_row(submission_id, data_submission_str):
    ...
    try:
        ...
        result = download_image(original_image_url, original_path, submission_id)
...
def main():
    parser = argparse.ArgumentParser(description='下载审核未通过的original图片')
    parser.add_argument(
        '--workers',
        type=int,
    ...
    # 加载审核未通过的submission_id
    print(f"\n读取审核结果...")
    rejected_ids = load_rejected_submission_ids()
    
    if len(rejected_ids) == 0:
        print("没有找到审核未通过的记录！")
        logger.warning("没有找到审核未通过的记录")
        return
    
    print(f"找到 {len(rejected_ids)} 个审核未通过的记录")
    ...
    if missing_ids:
        print(f"\n警告: 有 {len(missing_ids)} 个审核未通过的submission_id在CSV文件中找不到")
        logger.warning(f"有 {len(missing_ids)} 个审核未通过的submission_id在CSV文件中找不到")
    ...
    print(f"预计下载 {len(tasks)} 张original图片")
    
    # 并行下载
    print(f"\n开始下载...")
    logger.info(f"开始下载，共 {len(tasks)} 条记录，预计下载 {len(tasks)} 张图片")
            ...
            for future in as_completed(future_to_task):
                try:
                    ...
                    if result.get('status') == 'success':
                        ...
                        if 'error' in result:
                            error_msg = f"错误 [{result.get('submission_id', 'unknown')}]: {result['error']}"
                            print(f"\n{error_msg}")
                except Exception as e:
                    error_count += 1
                    error_msg = f"处理失败: {e}"
    ...
    if error_records:
        ...
        for sid, records in sorted(error_by_submission.items()):
            ...
            print(f"  - {sid}: {', '.join(errors[:2])}")  # 只显示前2个错误
