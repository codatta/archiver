...
"""
从旋钮_交付_2.csv下载图片：
1. 直接读取旋钮_交付_2.csv（所有记录都是审核通过的）
2. 下载original_image和annotated_image
3. 图片命名为: submission_id + "_original" 和 submission_id + "_annotated"
4. 下载的图片存储到images_0123目录下
"""
...
CSV_FILE = BASE_DIR / "旋钮_交付_2.csv"
OUTPUT_DIR = BASE_DIR / "images_0123"
LOG_DIR = BASE_DIR / "logs"
LOG_FILE = LOG_DIR / f"download_batch2_{datetime.now().strftime('%Y%m%d_%H%M%S')}.log"
...
def main():
    parser = argparse.ArgumentParser(description='下载第二批图片')
    parser.add_argument(
        '--workers',
        type=int,
    ...
    print(f"找到 {len(tasks)} 条记录（全部审核通过）")
